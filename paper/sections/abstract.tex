...
that the composed series inherited the generator's marginal variance
...
downstream consumers could audit which path it took. Evaluated on a
424-ticker US equity and exchange-traded fund (ETF) universe against
the SPY market path
using \texttt{JumpHMM.jl} marginals, the hybrid construction recovered
the factor loading as precisely as either baseline while preserving
the heavy-tailed marginal across the full empirical range of factor
loadings. All artifacts, scripts, and the fitted marginal cache are
