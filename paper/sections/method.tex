...
market growth-rate path $g_m \in \R^T$ over $T$ trading days
(formally introduced below, with $\teps_i$ assumed independent of
$g_m$ and $\E[\teps_i] = 0$) by
rescaling the generator draw as $\eps_i = s_i\,\teps_i$ with a
scalar $s_i \in (0,1]$ chosen to preserve one of two per-asset
marginal targets while recovering the calibrated single-index
model (SIM) coefficients $(\alpha_i, \beta_i)$ in the large-$T$
limit.
...
$\sigma_m^2 = \Var[g_m]$. In our experiments the market path is the
SPY total-return growth-rate series; the construction is agnostic to
whether $g_m$ is a single-name proxy, a synthetic index, or a
...
\texttt{JumpHMM.jl} v0.1.0 (commit
\texttt{9be8099})\footnote{\url{https://github.com/varnerlab/JumpHMM.jl}},
fit to the asset's own full growth-rate history (not to residuals
after market detrending) using the package-default EM tolerance
and maximum-iteration cap. Because the generator's variance reflects
...
sampled independently of $g_m$ (verified empirically across the
$423$-ticker universe in Fig.~\ref{fig:cov_diag}). If the generator
already carries non-trivial market correlation, the residual
covariance can be subtracted from $\beta_i$ before applying the
correction; we return to this point in the discussion.
...
coefficient of determination, used by the branch-selection step
defined below.
...
the empirical-copula choice in the results below.
...
\end{equation}
Under the independence assumption $\Cov[\teps_i, g_m] \approx 0$,
the cross term in \eqref{eq:variance-target} vanishes and
\begin{equation}
...
$g_i = \alpha_i + \beta_i g_m + \teps_i$ corresponds to $s_i = 1$ and
inflates $\Var[g_i]$ by exactly $\beta_i^2 \sigma_m^2$.
...
against SPY has $R^2 \approx 0.86$. For these tickers we require $\hat\beta$ and $R^2$ to recover their
calibrated values, neither inflated to one by dropping $\eps$ nor
implied by unrelated marginal-variance bookkeeping.
...
single names below $0.65$ (Fig.~\ref{fig:r2_distribution}), so
$R^2_{\mathrm{preserve}} = 0.80$ captures the trackers without
sweeping in any genuinely idiosyncratic name. For any
...
$R^2$-preserving construction of Eq.~\eqref{eq:r2-target}, and the
rest go through the variance-preserving construction of
Eq.~\eqref{eq:scale} with the clip of Eq.~\eqref{eq:clip} engaged
when the market loading ratio $\rho_i$ would otherwise exceed $1 - f$. Each asset is tagged
...
the variance algebra of Eq.~\eqref{eq:scale} is unaffected and
whatever cross-sectional structure is applied to $\eps_i$ passes
through to $g_i$ unchanged.
...
cross-sectional dependence) in the large-$T$ limit.
...
requires a richer construction, which we return to in the discussion. The pipeline (\texttt{Pipeline.jl}, in
the released code) is modest in compute: fitting the $423$ per-asset
...
runs in minutes on a 2023 MacBook Pro (Apple M2 Pro, 16~GB RAM),
and the full six-composer sweep reported below completes within an
