...
market path $g_m \in \R^T$ over $T$ trading days, with sample
variances $\sigma_{\gen,i}^2 = \Var[\teps_i]$ and
$\sigma_m^2 = \Var[g_m]$. The residual $\eps_i$ in
\eqref{eq:sim-composition} is obtained by rescaling the generator
draw, $\eps_i = s_i\,\teps_i$, with the scalar $s_i \in (0, 1]$ chosen
so that ordinary least squares (OLS) of $g_i$ on $g_m$ recovers the
calibrated single-index model (SIM) coefficients
$(\alpha_i, \beta_i)$ in the large-$T$ limit, and so that the composed
marginal satisfies one of two preservation targets. For assets whose
generator marginal carries the structure of interest (heavy tails,
regime switching, jumps), the variance-preserving branch picks $s_i$
so that $\Var[g_i] = \sigma_{\gen,i}^2$. For assets that track the
market closely, the $R^2$-preserving branch picks $s_i$ so that the
calibrated real-data coefficient of determination $R^2_{i,\real}$
(the OLS $R^2$ from the asset's real return history against the real
market path) is recovered exactly. Branch selection is driven by the
calibrated $R^2$ alone, so the procedure generalizes to any new
ticker without external metadata.
...
$g_i = \alpha_i + \beta_i^{\eff}\, g_m + \eps_i$ to satisfy three
...
quadratically in $\beta_i$. The rest of this section fixes this
inflation with a single closed-form scalar correction
(Section~\ref{sec:method-variance}), handles the high-$\beta$
degenerate case with a sign-preserving clip
(Section~\ref{sec:method-clipping}), treats index-tracking assets
through an $R^2$-preserving branch (Section~\ref{sec:method-r2}), and
ties the pieces together in an algorithm while stating the resulting
properties of the composed series
(Section~\ref{sec:method-algorithm}).
...
\end{equation}
When $\teps_i$ is uncorrelated with $g_m$ (true by construction for
independent draws, and true after a copula rank reorder when the copula is
sampled independently of $g_m$), the cross term in \eqref{eq:variance-target}
vanishes and
\begin{equation}
...
against SPY has $R^2 \approx 0.86$. For these tickers the practitioner cares
that (i) $\hat\beta$ recovers to its calibrated value, and (ii) $R^2$ recovers
to its calibrated value, neither inflated to one by dropping $\eps$ nor
...
Algorithm~\ref{alg:hybrid} ties the pieces together. For each asset
the calibrated $R^2_{i,\real}$ selects a branch: assets with
...
$\texttt{hybrid-clipped}$, or $\texttt{fallback}$ that is persisted
alongside the output, so downstream consumers can audit which path
each asset took. The optional rank reorder in line~14 accepts any
